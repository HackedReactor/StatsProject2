...
```{r}
full.log = glm(term_deposit~ age + job + marital + education + default + housing + loan + contact + month + day_of_week + duration + campaign + pdays + previous + poutcome + emp.var.rate + cons.price.idx + cons.conf.idx + euribor3m + nr.employed,family="binomial",data=smote_train)
step.log = full.log %>% stepAIC(trace=FALSE)
summary(step.log)
exp(cbind("Odds ratio" = coef(step.log), confint.default(step.log, level = 0.95)))
vif(step.log)
...
<<<<<<< HEAD
ex_step_log = glm(term_deposit ~ job + marital + education + housing + contact + month + day_of_week + campaign + previous + emp.var.rate + cons.price.idx + cons.conf.idx + euribor3m + nr.employed + age*job + job*marital, data = smote_train, family = "binomial")
summary(ex_step_log)
=======
