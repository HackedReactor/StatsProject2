...
#Simple Logistic regression model, by results from EDA
## job   marital  education  contact  month  day_of_week  poutcome

simple.final.log = glm(term_deposit ~ job + marital + education + contact + month + day_of_week + poutcome, data = smote_train, family = "binomial")
...
#simple model
simple.final.log = glm(term_deposit ~ job + marital + education + contact + month + day_of_week + poutcome, data = smote_train, family = "binomial")
fit.pred.simple = predict(simple.final.log, newdata = test, type = "response")
...
legend("bottomright",legend=c("Lasso","Stepwise","Simple Model Only"),col=c("black","orange","blue"),lty=1,lwd=1)
abline(a=0, b= 1)
```
...
Bank_Fix_Imp_Numeric <- cbind(Bank_Fix_Imp[,8:11],Bank_Fix_Imp[,13:15])
...
library(ROCR)
library(MASS)
mylda<- lda(term_deposit ~ pdays+campaign+euribor3m+nr.employed+cons.price.idx+cons.conf.idx, data = smote_train)
...
>>>>>>> a5239389b5d549bf261ba73231e141f5192eba04
...
p.mat <- cor.mtest(Bank_Conti[,2:10])
correlation <- cor(Bank_Conti[,2:10])
...
# ll.null <- full.log$null.deviance/-2
# ll.proposed <- full.log$deviance/-2
...
Testing with different predictors and checking accuracy
```{r}
#Testing the simple logistic model with different predictors as achieved from the EDA
simple.final.log = glm(term_deposit ~ job + marital + education + contact + month + day_of_week + poutcome + cons.price.idx*cons.conf.idx + nr.employed:euribor3m, data = smote_train, family = "binomial")
summary(simple.final.log)
